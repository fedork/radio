...
#
#   ./run_radio_oracle.sh [--max-k K] [--max-n N] [--max-part-n N]
#                         [--no-prime] [--journal FILE] [-- <cache files>]
#
# Sizing is a decision you make once, because the process is meant to stay up:
#
#   MAX_N must cover the sum of all component side-sums in any state you will ask about.
#   MAX_PART_N must cover the largest one-component side-sum; it defaults to MAX_N.  Separating the
#   two avoids building a huge component catalog for long states made of small groups.  Neither
#   bound has to cover the caches: the loader SKIPS facts it cannot represent and reports how many, so a narrow build
...
while (($#)); do
    case "$1" in
        ...
        --max-part-n) MAX_PART_N="$2"; shift 2 ;;
...
[[ -n "$MAX_PART_N" ]] || MAX_PART_N="$MAX_N"

BIN="./radio_oracle_k${MAX_K}_n${MAX_N}_p${MAX_PART_N}"
if [[ ! -x "$BIN" || radio_oracle.c -nt "$BIN" || radiobase.c -nt "$BIN" ]]; then
    echo "building $BIN (MAX_K=$MAX_K MAX_N=$MAX_N MAX_PART_N=$MAX_PART_N)" >&2
    tools/build_radio.py -O3 -DMAX_K="$MAX_K" -DMAX_N="$MAX_N" \
        -DMAX_PART_N="$MAX_PART_N" radio_oracle.c -o "$BIN" >&2
fi
