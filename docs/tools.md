...
| `tools/singleton_direct_split_cleanroom.cpp` | independently enumerate legal singleton row triples into three sorted child multisets, with no Hall code or shared cache; solve arbitrary first-cut queries and check the padded/core `K=6` hole with slack-correct residual bounds |
| `tools/singleton_direct_split_regression.sh` | provenance-build the clean-room direct-row solver and run its canonical `G_6`, `j=13`, padded/core `j=14`, naive-oracle and exhaustive `K<=3` controls |
