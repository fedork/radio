...
| `tools/singleton_pair_coloring_census.cpp` | exhaust all full-mass singleton partitions majorized by `G_K` through `K=4`, check the exact Row-Coloring Hall inequalities, compare forward and global-balance coloring rules, and uniformly sample the exact `K=5` partition universe |
| `tools/singleton_shape_survey.cpp` | reconstruct legal first cuts and test the scalar equal-average coalescence target or the stronger independently rounded hinge-profile target; exhausts the scalar rule through `K=4` and reproduces the `K=3` full-profile counterexample |
