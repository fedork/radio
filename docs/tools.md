...
| `tools/singleton_pair_coloring_census.cpp` | exhaust all full-mass singleton partitions majorized by `G_K` through `K=4`, check the exact Row-Coloring Hall inequalities, compare coloring rules, sample the exact `K=5` partition universe, check the padded Pascal-prefix reduction, and verify its compressed `K=19` strict-alternation counterexample |
| `tools/singleton_shape_survey.cpp` | reconstruct legal first cuts; test scalar/full-profile shape targets; enumerate feasible split-count fibers and endpoint obstructions; exhausts the scalar rule through `K=4` and the interval survey through `K=3` |
