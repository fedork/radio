...
| `tools/singleton_pair_coloring_census.cpp` | exhaust full-mass singleton partitions majorized by `G_K` through `K=4`; check Row-Coloring Hall inequalities, global Adjacent-Fiber transfers and coloring rules; inspect labelled fixed-boundary delta exchange, exact crossing blockers, and maximum/positive crossing-mass rules; sample the exact `K=5` universe; check the padded Pascal-prefix reduction and its compressed `K=19` strict-alternation counterexample |
