...
| `tools/singleton_tight_band_certificate.cpp` | emit deterministic no-first-cut certificates from two tight anchors, exact endpoint transitions, the forced mixed floor and pure-row capacities; contains no split search or Hall code and surveys the fixed `K=6` rank-15/32 face |
| `tools/singleton_tight_band_regression.sh` | provenance-build and run both the inequality-only tight-band certificate regression and the independent direct-row controls/face classification |
