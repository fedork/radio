...
| `tools/singleton_pair_coloring_census.cpp` | exhaust all full-mass singleton partitions majorized by `G_K` through `K=4`, check the exact Row-Coloring Hall inequalities, compare forward coloring rules, and uniformly sample the exact `K=5` partition universe |
