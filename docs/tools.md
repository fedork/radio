...
| `tools/singleton_pascal_interval_census.cpp` | exhaust exact-row contracted Pascal bands and arbitrary-row suffixes through `K=4`; check every incoming tight-prefix count, inspect endpoint transition matrices, count exact-support/tight-skeleton states, and verify the strict-prefix-interior `K=6` alternation counterexample |
