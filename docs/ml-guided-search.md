...
| **concentric round expansion — succeeds where coordinate descent failed** | `tools/ml/proto_concentric_rounds.py` | replaces `canSolveB_ctx`'s arbitrary CPU-unit cutoff with an interpretable one: split a state's segments into concentric ones (radius grown together each round, per-segment growth factor `G^(1/(P-1))` derived from first principles to avoid a naive `G^P` blowup) and a "last" segment always walked in full with the real pooled score. Tested with real oracle verification on all 4 tier-sample endpoints: **all 4 succeed**, including both where block coordinate descent failed decisively — exhaustive-within-radius search can't get stuck in the wrong basin the way restart-based local search can. Oracle-call cost stayed consistent (1,032-4,399) across a ~700x range in pooled-model difficulty (pooled rank 1 to 687), unlike coordinate descent's cheap-win-or-collapse pattern — a genuinely non-arbitrary, gracefully-degrading stopping signal. Not yet tested: propagating the round/radius down into the recursive verification of the children themselves (the multi-level half of the design). Benchmarked segment order against the actual `BY_MAGIC3` production heuristic (ported faithfully into Python) on the same 4 endpoints: no clean winner — magic3 wins on one endpoint (1.8x fewer calls), deficit wins on two (1.2x-1.8x), the fourth ties — segment order is a real but modest (~20-80%) efficiency lever, not what makes the method robust (all 4 succeed under either order). See [../evidence/concentric_round_search_2026-08-22.txt](../evidence/concentric_round_search_2026-08-22.txt) |
