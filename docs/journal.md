...
## 2026-09-04 -- run10 finished, and the "stall" was a 32-second archival race

The cold `Sa(193)` rerun **completed**, agreeing with run9:
...
`exit 1` is the negative-verdict success path, not a failure: `radio_sa193.c:59` returns
`r == TRUE ? 0 : r == FALSE ? 1 : 2`. All sixteen roots carry a printed `can't solve ... in 9`
line, the raw log passes `tools/check_provenance.py`, and `extract_evidence.py audit` finds 0
contradictions. 301,127.6 CPU seconds against run9's 419,353.1 is **28.19% cheaper**, now measured
rather than projected, at 1.05 GB peak RSS against 1.32 GB. Full identity, hashes, the sixteen-root
table and the forensic timeline are in the
[completion record](../evidence/run10_completion_2026-09-04.md); the artifacts are the release
`sa193-cold-run10-2026-09-04` and `run10/seg-20260831T232959Z/` in S3.

This is a second cold derivation by a different build, **not** an independent implementation. Same
engine lineage, commit `9e9e25a` with necessity-only cache semantics, different cache history. The
certificate of record and its independence properties are unchanged; what this adds is that current
main reproduces the verdict from cold, faster.

**Earlier today I recorded this run as having died at 13 of 16 roots.** That entry has been replaced
rather than annotated, because every operational conclusion in it was drawn from the last `STATUS`
object, which was a snapshot 8m 29s before the solver printed its result. Two of its numbers were
wrong and the second one is worth keeping as a lesson.

The first: the run was not 0.8% from the end, it was 105 wall seconds of *reporting* behind and
about 550 CPU seconds of work from done. `memprofile.csv`'s `solver_secs` column is **wall**
seconds since solver launch, not CPU -- its first row reads 20 at 20 seconds in, and its last,
301,023, matches `capped_run`'s 301,536 s wall once the remaining 8.5 minutes are added. I had read
301,023 as CPU and compared it against a CPU total, which is why the arithmetic looked consistent
while being wrong.

The second: my 0.729-ratio extrapolation put the remaining tail at ~2,400 CPU seconds. The true
figure is 1,287. Roots 1-12 decay by a stable 0.64-0.75, but the last four fall
1,317 -> 789 -> 390 -> 108, that is by 0.60, 0.49, 0.28. **Near the diagonal the decay accelerates
sharply**, because the shared cache has already absorbed nearly everything those roots need, so any
mid-range extrapolation of an `Sa` tail overshoots. Worth remembering the next time a run's
remaining cost has to be estimated from a progress line.

The cause of the shutdown is now certain, and it is a leftover, not a defect in the launcher.
`tools/aws_shared_job_shutdown_when_idle.sh` was still running from the 2026-08-31 arrangement in
which the `K=6` census shared this instance with the Sa run. It polls every 60 s and powers the host
off once neither `radio_sa193` nor `radio-k6-survey.service` is alive. The census moved to its own
Spot ASG on 2026-09-01 and was stopped at 05:13 UTC, leaving `pgrep -x radio_sa193` as the loop's
only condition. It was installed by hand over SSM and **nothing in `tools/` or `docs/` recorded that
it had been installed**, so the census teardown had no reason to remove it. Its companion
`/usr/local/bin/shutdown`, which shadows the real binary to defer cloud-init while the census runs,
is also still in place and is now a pass-through.

    11:06:43.23  memprofile row; watchdog's 600 s cycle ticks
    ...
    11:15:15.99  capped_run footer; radio_sa193 gone
    11:16:11     radio-shared-shutdown[667410]: "Sa(193) and K=6 survey ended; stopping host"
    11:16:18     systemd-shutdown: syncing filesystems; journald stopped
    11:16:43     <- where the watchdog's next tick, and the final archive, would have been

**A 60-second poll beat a 600-second poll by 32 seconds, and that is the whole story of the loss.**
`sa193_watchdog.sh`'s solver-GONE branch is the only path that uploads the complete log, the final
checkpoint, the stderr file and the FINISHED mail, and it never ran. Nor did user-data's own
uploads, which sit behind `wait $SOLVER_WRAPPER` -- and that `wait` is behind the *foreground*
watchdog, so it is unreachable until the watchdog returns. Hence: no completion mail, an S3 log
frozen at the 10:35Z hourly snapshot ending 4m 37s before the verdict, and a run that looked
stalled while being finished. The design lesson is that the final archive must not depend on a
polling watchdog noticing the exit; the shell that waits on the solver should upload the log itself
...
`pareto9-frontier/20260903T011520Z/input/input-sa193.cache.zst`, and it still checks at
2,266,369 lines and sha256 `fe85b0df...`. The bucket is **not** versioned, so an overwrite there is
irreversible; both objects replaced were strict prefixes of what replaced them.

One thing not to over-read: the shutdown was harmless *this* time only because the volume survived.
