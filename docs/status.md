...
| | goal | state |
|---|---|---|
| **H1** | Publish | **Active; citation coverage and a public DOI are the blockers.** Read [publication-handover.md](publication-handover.md) first, then [novelty-audit.md](novelty-audit.md). The audit found that Aigner's identical threshold table ends with the false seven-test value 37, so the checked `Sa(38)` tree is a constructive correction as well as part of the extension. OEIS, indexed Chapter 2/Du--Hwang text, and two current nice-graph papers are checked; Scholar cited-by and broader coverage remain. Reproduction package is built and validated (`tools/make_repro_package.sh`); the draft in `paper/` is mechanically clean but predates the strongest results and needs restructuring around them. |
...
- **Sa sequence, k = 1..10** — proven maximal. Aigner 1986 Figure 5 and Aigner 1988 Figure 2.13
  publish the identical threshold only through seven tests and incorrectly end at 37; the
  unconditional `Sa(38)` witness constructively refutes that claim, while the audited 2026
  boundary pair makes 38 exact. No later correction has yet been located, but a cited-by pass
  remains before claiming priority. `Sa(192)` has independently verified witness
...
- **Exact small-`m` frontiers** — Aigner gives `m=2,3`; Li--Wu--Triesch gives `m=4` and the
  ...
  Chapter 3 scan independently verifies the graph model and its `m=2,3` results; indexed Chapter 2
