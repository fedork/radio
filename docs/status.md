...
| trap | why it matters |
|---|---|
| **Do not use arbitrary weak majorization by `G_k` as a positive singleton theorem above `K=5`.** | Necessity holds at every level; exhaustive proof establishes sufficiency through `K=5`, and the explicit `K=6` hole refutes it thereafter. `[canonical U_k]` and `[embedded G_k]` leaves remain unconditional because they delete edges from Aigner's explicit strategy. The main solver now uses majorization positively only at `K<=5`; at `K>=6` a nonembedded singleton follows ordinary exact recursion. Unmarked cache files replay positive facts neither for singleton states nor for possibly tainted ancestors. |
...
| **A final verdict must not imply unbounded cache population.** | The ordinary engine proved the 30-row K=6 core negative in 0.004 s, then the old permutation-expanded dominance insertion continued past 12.9 CPU minutes. Current insertions stop after 1,000,000 deterministic recursive nodes by default, retain the exact fact and any sound closure prefix, and print `cache=partial:` on truncation. This changes hit rate only, never the verdict. |
