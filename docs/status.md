...
separator case is real rather than a proof artifact.  Padding to `3^K` labelled slots gives an
...
| **Do not replace the global Adjacent-Fiber target by a same-color shortcut.** | The exact transfer census passes all 141,690,676 normalized `K=4` transfer types, but 889 require leaving the fiber in which donor and recipient have the same color.  The first is `(16,15,11,9,7,5,5,5,1^8)` with transfer `11->9`: no feasible same-color assignment exists, while the opposite-color assignment `(15,11,5,5,1^4)/(16,9,7,5,1^4)` works before and after.  The finite Adjacent-Fiber result is still not a uniform proof.  See the [exact census record](../evidence/singleton_adjacent_fiber_census_2026-08-27.md). |
