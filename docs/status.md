...
0. **Use the warm oracle for everything that needs many verdicts.** `radio_oracle.c` is new: it
   ...
   **Start it cold and journal what you compute** (`--journal FILE` appends verdicts in loader
   format, so one session primes the next). Priming from the archive runs at 304 facts/s, 685 after
   `tools/sort_cache.py` reorders it — a real 2.25x, still ~8.9 h for all 21.9M. Only 1.4% of
   inserts are redundant, so the cost is genuine dominance-closure work, not duplicate facts; the
   actual fix is a structural snapshot of the trie, scoped but not built.
