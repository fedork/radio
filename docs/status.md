...
| **A streamed `zstd -c | aws s3 cp -` archive can be silently truncated: verify before trusting, and before deleting the source.** | On 2026-09-05 two sbwalk logs uploaded that way decoded to 19.375 MiB and 300.375 MiB and then failed with `Data corruption detected`, while a compress-to-file-then-upload of the same live log was clean. The truncated objects keep their verdict lines but lose the `with [...]` strategies, which is exactly what witness reconstruction needs -- and the m=136 instance had already been terminated with its volume, so the strategy for `Sb(107:83)@9` is unrecoverable and `Sa(11) >= 327` cannot yet be verified. `sa193_watchdog.sh` uses the same streamed pattern and has not been audited for this. **Run `zstd -t` on an archived log before deleting its source**, and prefer `cp; zstd -o file; zstd -t; aws s3 cp file` as `tools/sbwalk_launch.sh` now does, reporting `log_upload_verified` in `STATUS`. |
| **The cache epoch marker is a per-file, sticky flag: concatenating a pre-refutation log behind a marked checkpoint promotes its positives to trusted.** | `parse_file` resets `cache_replay_accept_positive` at entry and sets it TRUE if *any* `# radio-cache-semantics=<current>` line appears anywhere in that file (`radiobase.c:3806`, `:3825`); positives are refused while it is FALSE (`:1287`, `:3154`). One file gets one verdict, so a file that begins with a marked checkpoint trusts everything appended after it. `k8-2026-05-12:out_k8.txt` carries no marker and loads negative-only alone -- but not when concatenated. Its negatives are sound regardless (majorization is rejection-only, necessity holds); its 184,649 positives predate the 2026-08-30 sufficiency refutation. To keep eras separate, keep the files separate and call `parse_file` per file, or strip `^+ ` lines before concatenating. The load prints `trusted` or `negative-only` -- read it. |
...
| | goal | state |
|---|---|---|
| **H1** | Publish | **Active; the public DOI is the only external-access blocker.** Read [publication-handover.md](publication-handover.md) first, then [novelty-audit.md](novelty-audit.md). Aigner's threshold table ends with the false seven-test value 37, but Gargano 1992's published `Sb(21:17)@6` already implies a construction for 38, and its full `Sb(32:32)@7` tree implies 64 in eight tests. The local 38/39 boundary is the exact correction; the tree is an independent certificate, not first achievability. The 69-work OpenAlex forward-citation union, 106 Semantic Scholar seed records, 424 broad OpenAlex discovery records, OEIS, modern same-model bibliographies, all 264 Scholar rows exposed for the four requested cited-by walks (227 deduplicated), Hwang 1989, and the available Du--Hwang target pages are checked. The access search is closed; Christen 1980/1986 and seven missing Du--Hwang pages are documented corpus limitations, not active work. The reproduction package is built and validated, and the manuscript has been restructured around the counterexample with corrected priority language. Final author/venue editing remains. |
...
- **Sa sequence, k = 1..10** — proven maximal. Aigner 1986 Figure 5 and Aigner 1988 Figure 2.13
  publish the identical threshold through seven tests and incorrectly end at 37. Gargano 1992's
  `Sb(21:17)@6` already implies 38-achievability, and its `Sb(32:32)@7` tree gives the prior
  eight-test lower bound 64. The unconditional `Sa(38)` witness is an independent construction,
  while the audited 2026 boundary pair makes 38 exact; the local exact 65 improves the located
  prior eight-test construction by one. No source in the audited corpus reaches 65, 112 or 192;
  the Scholar/Hwang pass is closed, but unqualified priority language remains unjustified while
  the Christen originals and some book pages are unavailable. `Sa(192)` has independently verified witness
...
- **Singleton necessity, a negative converse theorem, and two further theorems** — Singleton
  ...
  and is **published** as Aigner 1988 eq. (3.10) - cite it, do not claim it (attribution
  corrected 2026-09-02); the in-repo page is a re-proof in this project's graph
  reformulation, and it is
...
- **Exact small-`m` frontiers** — Aigner 1986 gives `m=2,3,4` and the isolated exact cell
  `n(4,6)=7`; Li--Wu--Triesch gives the piecewise `m=5` formula; the retained local replay gives
  the `m=6,k=9` upper boundary. Zhang--Berger--Massey and Gargano also publish several finite
  endpoint constructions, including exact costs for `Sb(21:17)@6` and `Sb(32:32)@7`, without
  proving all neighboring frontier negatives. Thus the
  K=9 column is exact through `m=6`, with values 512, 511, 503, 496, 481, 473. The Aigner 1988
  Chapter 3 scan independently verifies the graph model and its `m=2,3` results; Aigner 1986 is
  the correct source for `m=4`, whose 1988 exercise version is a typo. Indexed Chapter 2
...
5. **P5 structural rewrite delivered 2026-09-02.** The paper now states `Sa(10)=192` as a proven
   maximum, centers the exact `K=5/6` majorization boundary, incorporates the certified-verification
   architecture and audited priority corrections, and contains no unresolved theorem TODOs. Final
   venue formatting and author editing are publication work, not an H3 compute dependency.
