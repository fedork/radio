...
| | goal | state |
|---|---|---|
| **H1** | Publish | **Active, and the blocking item is prior art, not writing.** Read [publication-handover.md](publication-handover.md) first, then [novelty-audit.md](novelty-audit.md). Reproduction package is built and validated (`tools/make_repro_package.sh`); the draft in `paper/` is mechanically clean but predates the strongest results and needs restructuring around them. |
