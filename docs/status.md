...
| | goal | state |
|---|---|---|
| **H1** | Publish | **Active; four access checks and a public DOI are the blockers.** Read [publication-handover.md](publication-handover.md) first, then [novelty-audit.md](novelty-audit.md). Aigner's threshold table ends with the false seven-test value 37, but Gargano 1992's published `Sb(21:17)@6` already implies a construction for 38, and its full `Sb(32:32)@7` tree implies 64 in eight tests. The local 38/39 boundary is the exact correction; the tree is an independent certificate, not first achievability. The 69-work OpenAlex forward-citation union, 106 Semantic Scholar seed records, 424 broad OpenAlex discovery records, OEIS, indexed Du--Hwang text, and modern same-model bibliographies are checked. Full Du--Hwang, Hwang 1989, Christen 1980/1986, and a Scholar export remain. Reproduction package is built and validated (`tools/make_repro_package.sh`); the draft in `paper/` is mechanically clean but predates the strongest results and needs restructuring around them. |
...
- **Sa sequence, k = 1..10** — proven maximal. Aigner 1986 Figure 5 and Aigner 1988 Figure 2.13
  ...
  but four access checks remain before an unqualified priority claim. `Sa(192)` has independently verified witness
