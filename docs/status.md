...
transfer distance 14 is minimal on this face only.  Counts and provenance are in the
[capacity record](../evidence/singleton_tight_band_capacity_2026-08-31.md).
The next singleton work is to settle `K=5`, search beyond this fixed `K=6` face/support, and test
every new hole for a two-anchor certificate before escalating to laminar or general Hall-dual
certificates.  A useful exact recursive support criterion remains the longer-term target.  The
...
| **A missing tight-band certificate is not a positive result.** | The two-anchor capacity test is a sound sufficient obstruction only.  `certificates=0` means unknown.  The 175 positive verdicts on the fixed rank-15/32 face come from the independent direct-row solver and its replay check, not from the inequality-only extractor. |
...
| **H4** | Structural theory | The Singleton Majorization Converse is resolved negatively at `K=6`; the independent direct-row verifier and proved tight-band capacity certificate are delivered.  The counterexample is the unique first-cut hole on its fixed rank-15/32 face.  Settle `K=5` and search broader `K=6` faces/supports only if publication needs the stronger minimality statement.  The excess-`q` Pareto-assembly avenue remains parked. |
