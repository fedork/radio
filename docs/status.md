...
The cleaner remaining target is the Positive Pascal Crossing Lemma: some original-state-feasible
crossing flip or swap must strictly increase the mass on the recipient-opposite side.  Repeating
such moves is acyclic and proves Adjacent-Fiber, because the integer side mass strictly increases
until an endpoint moves or a common coloring is reached.  Complete `K=3` and a 5,000-state `K=4`
window have maximum crossing gain at least one at every failure; all maximum-gain ties actually
finish in one move there.  This is finite evidence, not the missing existence proof.  Row-count
slack and closest-boundary rules have `K=4` counterexamples, so the exchanged identities cannot be
