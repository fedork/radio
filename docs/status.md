...
don't know it. Last refreshed **2026-08-28**.  The central correction is that singleton
...
failure.  Exact flip/swap formulas instead show that a stopped search alternates between
`B`-heavy and `A`-heavy blocker cuts; every successful local repair must cross the original tight
separator, where the Tight Pascal-Band Lemma gives at least two units of rank gain.  Plain strict
dyadic-band descent is false: a `K=3` crossing swap has a blocker with exactly the original band.
The cleaner remaining target is now the Core--Blocker Escape Lemma.  Dangerous tight separators
...
blocker, so no alternating cut chain or cyclic reassignment is needed.  Complete `K=3` and a
5,000-state `K=4` window support this statement indirectly: maximum crossing gain is at least one
at every failure and every maximizing tie finishes in one move.  This is finite evidence, not the
missing existence proof.  Row-count slack and closest-boundary rules have `K=4` counterexamples,
so the exchanged identities cannot be fixed in advance.  Boolean labels alone do not imply the
needed exchange, and laminar supports are false already at `K=3`.  Padding to `3^K` labelled slots gives an
...
| **Do not promote the finite singleton-coloring census or a shape/counting rule to a theorem.** | Exhaustion proves the Row-Coloring Lemma only through `K=4`.  Balanced value blocks with one-block lookahead pass ten million uniform `K=5` samples, but `K=5` has 38,378,683,542,323 full-mass dominated states.  Padding and labelling do not make raw counting sufficient: child triples map many-to-one to parents with nonconstant fibers already at `K=1`, so counting triples gives coefficient mass rather than support size.  Padding does give exact pure-first and equal-block allocation problems, but the fixed alternating-interval implementation fails at `K=3` for `(8,7,4,1^8)`.  Canonical-tail alternation reduces the high-support case to one explicit prefix inequality, but the finite `K<=12` pattern is deceptive: the inequality fails at `K=19`, and an exact compressed full-mass parent realizes a Hall excess of 2,431.  The scalar equal-average coalescence rule also passes all 5,997,038 `K=4` states, and `Q_3` is exhaustively strongly nice on 463,886 comparable pairs, but both are finite observations, not certificates.  Independently matching the whole normalized hinge profile is false already at `K=3`, for `(8,5,5,5,1^4)`: the coordinates have coupled lattice constraints.  Requiring every row image to contain a piece whose width occurs in `G_(K-1)` is also false at `K=3`: `(8,7,4,2,2,2,1,1)` forces at least ten units into a mixed branch of mass nine.  Strict alternation already fails at `K=3` on `(8,7,4,2,2,1^4)`; adjacent-pair orientation fails on 916 `K=4` states; weaker block rules have sampled `K=5` counterexamples; even globally minimizing total-mass difference subject to the necessary row counts fails at the 15,855th enumerated `K=4` state.  The clean exact target remains Pascal orthant saturation/no lattice holes, with special-operator strong niceness and adaptive augmenting paths as stronger routes.  The general converse is open; see [the theorem note](theorems/singleton-majorization.md#a-stronger-chromatic-symmetric-function-target-2026-08-27), [padded reduction](../evidence/singleton_padded_three_blocks_2026-08-27.md), [shape survey](../evidence/singleton_shape_survey_2026-08-27.md), [strong-niceness census](../evidence/singleton_strong_niceness_2026-08-27.md), and [coloring census](../evidence/singleton_row_coloring_census_2026-08-26.md). |
| **Do not replace the global Adjacent-Fiber target by a same-color shortcut, delta-matroid exchange, strict band descent, or a preselected crossing row.** | The exact transfer census passes all 141,690,676 normalized `K=4` transfer types, but 889 require leaving the fiber in which donor and recipient have the same color.  They are exactly `(16,15,11,9,lambda)` with transfer `11->9`; the first four rows already forbid same-coloring.  The [Tight Pascal-Band Lemma](theorems/singleton-majorization.md#pascal-first-boolean-columns-and-a-tight-band-lemma-2026-08-28) proves that every dangerous separator crosses some repeated internal rank, but fixed-color switches cannot defeat a tight Hall cut.  Moving among feasible colorings is essential.  After the correct equal-row and global-complement quotient, every failed coloring through `K=3` and in four disjoint `K=4` windows totaling 13,000 states reaches success by one row flip or one opposite-color swap.  This mirrors support-two exchange in the ambient bisubmodular polyhedron, but the fixed-absolute-value slice does not even satisfy symmetric exchange: `(3,2^11,1,1)@3` is an exact counterexample.  A blocker can also repeat the same Pascal band, so band width alone is not a descent potential.  The live target is the Core--Blocker Escape Lemma: cross the intersection of all dangerous tight sets and choose the returning row from the intersection of all flip blockers outside their union.  Positivity proves directly that the returning-row side has no blocker; do not build another alternating-cut descent.  Complete `K=3` and the first 5,000 `K=4` states support the target with minimum maximum gain one, but that is evidence, not a theorem.  Row-count slack and two closest-boundary swap rules also fail at `K=4`.  See the [exact census and construction](../evidence/singleton_adjacent_fiber_census_2026-08-27.md), [coloring-landscape record](../evidence/singleton_coloring_landscape_2026-08-28.md), and [boundary-exchange record](../evidence/singleton_boundary_exchange_2026-08-28.md). |
...
| **Do not use the old Sa(113) colored certificate as proof.** | The independent checker reported its 120,302 records closed, but the frozen solver-core refuter exposes nine uncovered splits while closing all 304,105 normalized facts. The first gap is `Sb(15:8,8:5,8:5)@5`; the discrepancy in `radio_verify.c` coloring/replay is undiagnosed. The new run9 coloring path does not trust that subset: it starts from complete level support, records the exact production-trie terminal cited by every rejection, emits a selection only after zero gaps, and independently audits every selected next level. |
