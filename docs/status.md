...
`K=3`.  These expose useful conjectural geometry but supply no general proof.
...
| **Do not promote the finite singleton-coloring census or a shape rule to a theorem.** | Exhaustion proves the Row-Coloring Lemma only through `K=4`.  Balanced value blocks with one-block lookahead pass ten million uniform `K=5` samples, but `K=5` has 38,378,683,542,323 full-mass dominated states.  The scalar equal-average coalescence rule also passes all 5,997,038 `K=4` states, but it is only a stronger finite observation, not a certificate.  Independently matching the whole normalized hinge profile is false already at `K=3`, for `(8,5,5,5,1^4)`: the coordinates have coupled lattice constraints.  Adjacent-pair orientation fails on 916 `K=4` states; weaker block rules have sampled `K=5` counterexamples; even globally minimizing total-mass difference subject to the necessary row counts fails at the 15,855th enumerated `K=4` state.  The clean exact target remains Pascal orthant saturation/no lattice holes.  The general converse is open; see [the theorem note](theorems/singleton-majorization.md#a-shape-preserving-forest-target-2026-08-27), [shape survey](../evidence/singleton_shape_survey_2026-08-27.md), and [coloring census](../evidence/singleton_row_coloring_census_2026-08-26.md). |
