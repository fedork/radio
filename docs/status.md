...
by one and must cross a repeated internal Pascal rank.  What remains is a Dyadic Plateau-Descent
Lemma showing that alternating switches can reach that rank or expose a strictly narrower
separator.  Boolean labels alone do not imply the needed shadow closure, and laminar supports are
...
| **Do not replace the global Adjacent-Fiber target by a same-color shortcut.** | The exact transfer census passes all 141,690,676 normalized `K=4` transfer types, but 889 require leaving the fiber in which donor and recipient have the same color.  They are exactly `(16,15,11,9,lambda)` with transfer `11->9`; the first four rows already forbid same-coloring.  This is not a small-level accident: for every `K>=4` a proved family gives `K-3` analogous recipient values.  The crossing repair works by moving the two mixed contributions beneath an identical Pascal pair.  The [Tight Pascal-Band Lemma](theorems/singleton-majorization.md#pascal-first-boolean-columns-and-a-tight-band-lemma-2026-08-28) now proves that every dangerous separator crosses some repeated internal rank, but it does not prove that alternating switches can traverse that rank.  The Dyadic Plateau-Descent Lemma remains open.  See the [exact census and construction](../evidence/singleton_adjacent_fiber_census_2026-08-27.md). |
