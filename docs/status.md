...
| **Do not promote the finite singleton-coloring census or sampled block rule to a theorem.** | Exhaustion proves the Row-Coloring Lemma only through `K=4`.  Balanced value blocks with one-block lookahead also pass ten million uniform `K=5` samples, but `K=5` has 38,378,683,542,323 full-mass dominated states.  Adjacent-pair orientation fails on 916 `K=4` states; weaker block rules have sampled `K=5` counterexamples; even globally minimizing total-mass difference subject to the necessary row counts fails at the 15,855th enumerated `K=4` state.  The clean target is now Pascal orthant saturation/no lattice holes, not another scalar balance rule.  The general converse remains open; see [the theorem note](theorems/singleton-majorization.md#global-signed-lifting-and-the-no-holes-target-2026-08-26) and [census record](../evidence/singleton_row_coloring_census_2026-08-26.md). |
