...
| [`pareto8-certificate-2026-09-03`](https://github.com/fedork/radio-data/releases/tag/pareto8-certificate-2026-09-03) | **the `K=8` `Sb` Pareto frontier certificate of record** — one flat `radio-negative-certificate-v1` carrying all 11,215,465 claims over `k=1..8`, plus a metadata tar of the AWS `verify.out`, `final.sha256`, stage/bootstrap/smoke/selftest logs, the local `k<=4`/`k=6` audit logs, the level-v2 `k=8` certificate and a README. See [../evidence/pareto8_certificate_2026-09-03.md](../evidence/pareto8_certificate_2026-09-03.md) | 30 M | Its 54 `k=8` roots `Sb(57:55)..Sb(256:2)` are exactly the `n1+1` refutation for every `proven-exhaustive` `k=8` row of `data/pareto_sb.csv`. Cleanroom audited end to end on a `c8a.24xlarge` in 6h01m35s: `TOTAL verified 11215465, gaps 0, cells 13052810819342`, exit 0, zero `GAP` lines, and the `--expect-claims` gate passed so every claim is accounted for. Because the flat file carries every level the run re-derived all of them, and each level that also ran locally reports an **identical cell count** (`k=4` 1,787,253,880, `k=6` 92,620,060,382, `k=7` 45,619,114) — the same deterministic work on different hardware. **Deliberately excluded:** the level-v2 packagings of `k=6` (199 M) and `k=7` (52 M), which carry nothing the flat certificate lacks and regenerate in minutes via `tools/make_refute_level_certificate.py`; and the 1.33 GB source `out_k8.txt`, which predates the provenance banner and so is not archivable as raw evidence — it was never the trust base, since the cleanroom shares no solver code. Certifies the **negative** side only; achievability for all 55 cells rests on `witnesses/`, checked by `tools/check_witness.py`. |
...
| | raw | why |
|---|---|---|
| oracle cache snapshot `oracle-prime/20260820T165448Z/cache.snap.zst` | 667 M compressed, 6.67 G raw | **Historical derived artifact in S3 only; do not restore into a current proof/search run.** It contains 21,866,180 mixed-sign cache facts accumulated before singleton-majorization sufficiency was refuted, and the origin of individual positive dominance hits is not separable. The old restore check against 2,200 independently labelled states and the 32.8-second / 2.41-GB performance measurement remain valid for that sample and build; they do not make the opaque cache proof-safe. Rebuild current `main` cold or load only provenance-separated surviving inputs such as the negative `Sa(193)` certificate. |
