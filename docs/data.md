...
| [`sa193-cold-run10-2026-09-04`](https://github.com/fedork/radio-data/releases/tag/sa193-cold-run10-2026-09-04) | the completed **second** cold `Sa(193)` derivation: raw log, stderr, and a metadata bundle with the frozen binary, source, provenance sidecar, memory profile and run.meta | 45 M | `Sa(193) = UNSOLVABLE` in 301,127.6 CPU s (28.19% under run9), 1.05 GB peak RSS, all 16 roots refuted, `Sa(192)` control passed, `check_provenance.py` OK, 0 audit contradictions; raw log sha256 `5622c3f39a68291138a91703f733ddffc49ee859d3ac17c66fd2d933d91215d8` and the release round-trips through `artifacts.sh verify`. A second derivation by a different build (`9e9e25a`, necessity-only semantics), **not** an independent implementation. Recovered from the instance volume after a leftover watcher powered the host off 63 s post-verdict; see the [completion record](../evidence/run10_completion_2026-09-04.md). |
