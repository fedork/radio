...
- **The singleton base sequence.** Aigner's star-forest sequence `N(k)` has the same entries as this project's `G_k`: `(1)`, `(2,1)`, `(4,3,1,1)`, `(8,7,4,4,1,1,1,1)`, ... (book p. 147, PDF p. 29).  Proposition 3.24 proves `M(N(k)) = k` by a three-way recursive test (book p. 148, PDF p. 30).  This is the historical source of the canonical singleton construction; the current exact singleton characterization is stronger.
- **Majorization's historical boundary.** Proposition 3.25 proves only the necessary direction: a star forest solvable in `k` is weakly dominated by `N(k)` (book pp. 149--150, PDF pp. 31--32).  The next paragraph explicitly says the converse was then open.  Do not attribute this repository's singleton-majorization iff theorem to Aigner 1988.
