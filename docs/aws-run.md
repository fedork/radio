...
| active `Sb` walk | run `20260904T205613Z` on on-demand `i-0f8c55d221d7f5396`, `r7iz.xlarge`, us-west-2b; **one** `radio_sb_walk` walker at `k=10`, `n1=192`, `m_start=136` ascending to `m_end=160`, capped at 7 days / 24 GiB; warm-started from a 3,801,370-fact cache (sha256 `9623a4e6...`) = run10's final checkpoint + 338,869 facts at mass 194-330 from the local probes + 47,373 harvested from the superseded five-walker run. Decides `Sa(11)` via max{m : n(10,m) >= 192}. Read with `tools/sbwalk_status.sh [--live]`. The five-walker run `20260904T202016Z` was terminated after ~40 min with no cell decided; its facts are in this cache. |
