...
| completed `Sa` job | `run10` cold `Sa(193)` rerun, on-demand `i-0318c3349a0df835b`, `r7iz.xlarge`, launched 2026-08-31 23:29:01 UTC, **finished 2026-09-04 11:15:15 UTC**: `Sa(193) in 10 = UNSOLVABLE (301127.6 s)`, all 16 roots refuted, `Sa(192)` control 389.9 CPU s, 1.05 GB peak RSS. The host was powered off at 11:16:18 by a leftover shared-host idle-shutdown watcher, 32 s before the watchdog's final archive; the complete log was recovered from the volume on 2026-09-04, verified and archived to release `sa193-cold-run10-2026-09-04`. 83.79 instance-hours at the verified $0.372/h, about $31. **Terminated** 2026-09-04 after the release round-tripped through `artifacts.sh verify`; root volume `vol-020083ad3e3df88c4` was deleted with it (`DeleteOnTermination=true`, confirmed `InvalidVolume.NotFound`). Snapshot `snap-03f0ad37ce7ec286d` (50 GiB, completed) is retained as the pre-recovery image. See the [completion record](../evidence/run10_completion_2026-09-04.md). |
...
| active `Sb` walk | run `20260904T221247Z` on on-demand `i-03f8c577e8d771049`, `r7iz.xlarge`, us-west-2b; one `radio_sb_walk` walker at `k=10`, `n1=192`, `m_start=135` ascending to `m_end=160`, capped at 7 days / 24 GiB; warm-started from a **15,459,274-fact** cache (sha256 `e0e24f51...`) = run10's final checkpoint + local probe facts at mass 194-330 + the live `pareto9` checkpoint + all of `k8-2026-05-12:out_k8.txt` + harvests from the two superseded runs. Decides `Sa(11)` via max{m : n(10,m) >= 192}. Read with `tools/sbwalk_status.sh [--live]`. Supersedes the m=136 run `20260904T205613Z` and the five-walker run `20260904T202016Z`, both terminated with no cell decided and their facts harvested into this cache. |
...
| `run10/` | post-refutation current main (`9e9e25a`) | 2026-08-31 23:29:01 | **completed second cold derivation**: `Sa(193) = UNSOLVABLE`, 301,127.6 CPU s (28.19% under run9), 1.05 GB peak RSS, 16/16 roots, control passed, provenance OK, 0 audit contradictions; raw log 470,935,285 B sha256 `5622c3f3...`; build `54419a4988bb53065d8855cd66d09e6f133896816aecdea635692c0ef33a7492` |
