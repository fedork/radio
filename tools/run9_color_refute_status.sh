#!/usr/bin/env bash
# One-shot status for the dedicated top-down colored replay.
set -euo pipefail
...
if [[ -n "$RUN_ID" ]]; then
    filters=(Name=tag:Purpose,Values=run9-colored-refute Name=tag:RunId,Values="$RUN_ID")
else
...
if [[ -z "$row" || "$row" == None* ]]; then
    echo 'no matching colored-refute instance found'
    exit 1
fi
read -r instance state itype launched discovered <<<"$row"
[[ -n "$RUN_ID" ]] || RUN_ID=$discovered
prefix="run9-colored-refute/$RUN_ID"
printf 'run9 top-down colored replay  run_id=%s instance=%s state=%s type=%s launched=%s\n' \
