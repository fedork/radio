...
rg -q 'block Pareto index: size=2, min level=2 facts, 2 blocks' "$work_dir/minimal-block.out"
rg -q 'TOTAL         4 facts          2 minimal  \(50\.0% redundant\)' "$work_dir/minimal-block.out"
rg 'np=|  TOTAL' "$work_dir/minimal-plain.out" > "$work_dir/minimal-plain.summary"
rg 'np=|  TOTAL' "$work_dir/minimal-block.out" > "$work_dir/minimal-block.summary"
