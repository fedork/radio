#!/usr/bin/env bash
# Top-down colored run9 replay on a dedicated EC2 host.  Each level audits only facts cited above.
set -euo pipefail
...
MAX_PROJECTED_CPU_SECONDS=440000
...
[[ "$WORK" == "/root/run9-colored-refute-$RUN_ID" ]] || usage
[[ "$PREFIX" == "run9-colored-refute/$RUN_ID" ]] || usage
...
current_output() {
    ...
    case "$value" in
        ...
        COLOR_K[2-9]) echo "color-k${value#COLOR_K}.out" ;;
        *) echo bootstrap.out ;;
    esac
}

write_status() {
    local state=$1 tmp out pid expected completed_levels=0 completed_claims=0
    tmp=$(mktemp "$WORK/STATUS.tmp.XXXXXX")
    out=$(current_output)
    pid=$(cat verifier.pid 2>/dev/null || true)
    expected=$(cat current.expected 2>/dev/null || echo 0)
    if [[ -s color-chain.tsv ]]; then
        completed_levels=$(awk 'NR > 1 {n++} END {print n+0}' color-chain.tsv)
        completed_claims=$(awk 'NR > 1 {s += $2} END {print s+0}' color-chain.tsv)
    fi
    {
        printf 'run9 top-down colored level-v2 negative-trie replay\n'
        ...
        printf 'top_level=%s\ntop_claims=%s\ncompleted_levels=%s\ncompleted_claims=%s\ncurrent_expected=%s\n' \
            "$TOP_LEVEL" "$TOP_CLAIMS" "$completed_levels" "$completed_claims" "$expected"
        ...
        [[ -s "$out" ]] && {
            ...
            grep '^COLOR_SELECTION ' "$out" | tail -n 1 || true
        }
        [[ -s color-chain.tsv ]] && {
            printf 'chain_header=%s\n' "$(head -n 1 color-chain.tsv)"
            tail -n 8 color-chain.tsv | sed 's/^/chain=/'
        }
        [[ -s benchmark.projection ]] && cat benchmark.projection
        [[ -s exit.status ]] && printf 'exit_status=%s\n' "$(<exit.status)"
        [[ -s color.total ]] && printf 'result=%s\n' "$(<color.total)"
        printf 's3=s3://%s/%s/\n' "$BUCKET" "$PREFIX"
    } > "$tmp"
    mv "$tmp" STATUS
}
...
upload_live() {
    ...
    tmp=$(mktemp "$WORK/PROGRESS.tmp.XXXXXX")
    {
        printf 'updated_utc=%s\n' "$(date -u +%FT%TZ)"
        [[ -s "$out" ]] && grep -E \
            '^(CACHE_PROGRESS|CACHE_DONE|FREEZE_PROGRESS|FREEZE_DONE|FROZEN_EPOCH|BATCH_START|BATCH_DONE|PROGRESS|TOTAL|COLOR_SELECTION)' \
            "$out" | tail -n 160 || true
    } > "$tmp"
    mv "$tmp" PROGRESS
...
run_phase() {
    ...
    shift 3
    "$SOURCE_DIR/tools/capped_run.sh" --seconds "$PHASE_SECONDS" --rss-gb "$RSS_GIB" \
        --label "$label" -- taskset -c "$CPUSET" "$@" > "$out" 2> "$err" &
    local wrapper=$!
    printf '%s\n' "$wrapper" > wrapper.pid
    : > verifier.pid
    local pid=
...
archive_level() {
    local level=$1 file checksum="color-k${level}.sha256"
    for file in "run9-k${level}.cert" "color-k${level}.selection" \
            "color-k${level}.out" "color-k${level}.err"; do
        [[ -f "$file" ]] || continue
        [[ -f "$file.zst" ]] || zstd -T1 -8 -f "$file" -o "$file.zst"
    done
    : > "$checksum"
    for file in "run9-k${level}.cert.zst" "color-k${level}.selection.zst" \
            "color-k${level}.out.zst" "color-k${level}.err.zst" \
            "color-k${level}-provenance.txt"; do
        [[ -f "$file" ]] || continue
        sha256sum "$file" >> "$checksum"
    done
    for file in "run9-k${level}.cert.zst" "color-k${level}.selection.zst" \
            "color-k${level}.out.zst" "color-k${level}.err.zst" \
            "color-k${level}-provenance.txt" "$checksum"; do
        [[ -f "$file" ]] || continue
        aws s3 cp "$file" "s3://$BUCKET/$PREFIX/levels/$file" --no-progress
    done
    aws s3 cp color-chain.tsv "s3://$BUCKET/$PREFIX/color-chain.tsv" --no-progress
...
preserve_final() {
    ...
    for file in test.out test.err bench.out bench.err bootstrap.out; do
        [[ -f "$file" ]] || continue
        [[ -f "$file.zst" ]] || zstd -T1 -5 -f "$file" -o "$file.zst" || true
    done
    : > final.sha256
    for file in "$BINARY" "$BINARY.provenance" benchmark.projection corpus-level-counts.tsv \
            color-chain.tsv color.total test.out.zst test.err.zst bench.out.zst bench.err.zst \
            bootstrap.out.zst bench-provenance.txt; do
        [[ -f "$file" ]] || continue
        sha256sum "$file" >> final.sha256
    done
    upload_live finished
    for file in STATUS PROGRESS run.meta exit.status final.sha256 benchmark.projection \
            corpus-level-counts.tsv color-chain.tsv color.total "$BINARY" "$BINARY.provenance" \
            test.out.zst test.err.zst bench.out.zst bench.err.zst bootstrap.out.zst \
            bench-provenance.txt; do
        [[ -f "$file" ]] || continue
...
awk '/^(fact|root) / {count[$2]++} END {for (k=1;k<=10;k++) if (count[k]) print k, count[k]}' \
    run9.cert > corpus-level-counts.tsv
[[ "$(awk '$1 == 9 {print $2}' corpus-level-counts.tsv)" == "$TOP_CLAIMS" ]]
...
python3 "$SOURCE_DIR/tools/build_radio.py" -O3 -pthread -Wall -Wextra -Wpedantic \
    -DMAX_K=10 -DMAX_N=194 -DRADIO_REFUTE_ENABLE_COLORING \
    "$SOURCE_DIR/radio_refute.c" -o "$WORK/$BINARY"
python3 "$SOURCE_DIR/tools/check_provenance.py" "$BINARY.provenance"

python3 "$SOURCE_DIR/tools/make_refute_level_certificate.py" run9.cert --level 7 \
    -o bench-k7.cert
set_stage BENCH
printf '%s\n' "$BENCH_FACTS" > current.expected
run_phase run9-colored-refute-bench bench.out bench.err \
    env RADIO_RUN_CONTEXT="run_id=$RUN_ID; stage=bench" REFUTE_THREADS="$THREADS" \
        ...
        stdbuf -oL -eL "$SOURCE_DIR/tools/run_with_provenance.py" \
            "./$BINARY" "$WORK/bench-k7.cert" "$WORK/bench-k6.selection"
"$SOURCE_DIR/tools/check_provenance.py" bench.out > bench-provenance.txt
grep -Eq "^TOTAL verified $BENCH_FACTS, gaps 0," bench.out
grep -Eq '^COLOR_SELECTION parent_level=7 selected_level=6 support=388317 used=[0-9]+ ' bench.out
...
[[ "$bench_done" == "$BENCH_FACTS" && "$bench_wall" =~ ^[0-9]+([.][0-9]+)?$ \
   && "$bench_cpu" =~ ^[0-9]+([.][0-9]+)?$ ]]
projected_wall=$(awk -v x="$bench_wall" -v sample="$BENCH_FACTS" -v total="$K7_FOUR_PART_FACTS" \
    'BEGIN {printf "%.0f", x*total/sample}')
projected_cpu=$(awk -v x="$bench_cpu" -v sample="$BENCH_FACTS" -v total="$K7_FOUR_PART_FACTS" \
    'BEGIN {printf "%.0f", x*total/sample}')
{
    printf 'sample_facts=%s\nsample_wall_seconds=%s\nsample_cpu_seconds=%s\n' \
        "$BENCH_FACTS" "$bench_wall" "$bench_cpu"
    printf 'projected_full_k7_four_part_wall_seconds=%s\nprojected_full_k7_four_part_cpu_seconds=%s\n' \
...
rm -f bench-k7.cert bench-k6.selection

printf 'level\taudited\tsupport\tused\tcitation_hits\twall_seconds\tcpu_seconds\tprefixes\n' \
    > color-chain.tsv
python3 "$SOURCE_DIR/tools/make_refute_level_certificate.py" run9.cert --level "$TOP_LEVEL" \
    -o "run9-k${TOP_LEVEL}.cert"
terminal_level=
for level in $(seq "$TOP_LEVEL" -1 "$BOTTOM_LEVEL"); do
    cert="run9-k${level}.cert"
    selection="color-k${level}.selection"
    out="color-k${level}.out"
    err="color-k${level}.err"
    expected=$(awk '/^claims / {print $3}' "$cert")
    [[ "$expected" =~ ^[1-9][0-9]*$ ]]
    printf '%s\n' "$expected" > current.expected
    set_stage "COLOR_K$level"
    run_phase "run9-colored-refute-k$level" "$out" "$err" \
        env RADIO_RUN_CONTEXT="run_id=$RUN_ID; stage=color-k$level" \
            REFUTE_THREADS="$THREADS" REFUTE_PROGRESS_SECONDS="$PROGRESS_SECONDS" \
            REFUTE_MIN_K="$level" REFUTE_MAX_K="$level" \
            stdbuf -oL -eL "$SOURCE_DIR/tools/run_with_provenance.py" \
                "./$BINARY" "$WORK/$cert" "$WORK/$selection"
    "$SOURCE_DIR/tools/check_provenance.py" "$out" > "color-k${level}-provenance.txt"
    grep -Eq "^TOTAL verified $expected, gaps 0," "$out"
    [[ "$(awk '$1 == "parent-level" {print $2}' "$selection")" == "$level" ]]
    next_level=$((level - 1))
    [[ "$(awk '$1 == "selected-level" {print $2}' "$selection")" == "$next_level" ]]
    [[ "$(awk '$1 == "source-claims" {print $2}' "$selection")" == "$expected" ]]
    [[ "$(awk '$1 == "audited" {print $2}' "$selection")" == "$expected" ]]
    support=$(awk '$1 == "support" {print $2}' "$selection")
    used=$(awk '$1 == "used" {print $2}' "$selection")
    hits=$(awk '$1 == "citation-hits" {print $2}' "$selection")
    source_support=$(awk -v k="$next_level" '$1 == k {print $2}' corpus-level-counts.tsv)
    source_support=${source_support:-0}
    [[ "$support" == "$source_support" && "$used" =~ ^[0-9]+$ && "$hits" =~ ^[0-9]+$ ]]
    [[ "$(grep -c '^use ' "$selection" || true)" == "$used" ]]
    wall=$(awk '/^BATCH_DONE / {for(i=1;i<=NF;i++) if($i ~ /^wall_s=/) {split($i,a,"="); print a[2]}}' "$out" | tail -n 1)
    cpu=$(awk '/^BATCH_DONE / {for(i=1;i<=NF;i++) if($i ~ /^cpu_s=/) {split($i,a,"="); print a[2]}}' "$out" | tail -n 1)
    prefixes=$(awk '/^TOTAL verified / {print $NF}' "$out")
    [[ "$wall" =~ ^[0-9]+([.][0-9]+)?$ && "$cpu" =~ ^[0-9]+([.][0-9]+)?$ \
        && "$prefixes" =~ ^[0-9]+$ ]]
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$level" "$expected" "$support" "$used" "$hits" "$wall" "$cpu" "$prefixes" \
        >> color-chain.tsv
    archive_level "$level"
    if (( used == 0 )); then
    ...
    (( next_level >= BOTTOM_LEVEL )) || {
        echo "color chain requested unsupported level $next_level" >&2
        exit 77
    }
    python3 "$SOURCE_DIR/tools/make_refute_level_certificate.py" run9.cert \
        --level "$next_level" --selection "$selection" -o "run9-k${next_level}.cert"
    [[ "$(awk '/^claims / {print $3}' "run9-k${next_level}.cert")" == "$used" ]]
done

[[ "$terminal_level" =~ ^[2-9]$ ]]
levels=$(awk 'NR > 1 {n++} END {print n+0}' color-chain.tsv)
audited=$(awk 'NR > 1 {s += $2} END {print s+0}' color-chain.tsv)
printf 'TOP_DOWN_COLOR verified_top=%s levels=%s audited=%s terminal_level=%s terminal_used=0 gaps=0\n' \
    "$TOP_CLAIMS" "$levels" "$audited" "$terminal_level" > color.total
