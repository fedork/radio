...
[[ "$active" == 0 ]] || { echo 'a run9-frozen-refute instance is already active' >&2; exit 69; }
...
bundle=$(mktemp "/tmp/radio-refute-${short}.XXXXXX")
userdata=$(mktemp "/tmp/radio-refute-userdata.XXXXXX")
...
cat > "$userdata" <<EOF
#!/bin/bash
exec > >(tee -a /var/log/run9-refute-bootstrap.log) 2>&1
set -euo pipefail
trap 'rc=\$?; aws s3 cp /var/log/run9-refute-bootstrap.log s3://$BUCKET/$prefix/bootstrap-failure.log --no-progress || true; shutdown -h +2 >/dev/null 2>&1 || true; exit \$rc' EXIT
systemd-run --unit=run9-refute-hard-stop --on-active=36h /sbin/shutdown -h now
...
trap - EXIT
exec source/tools/run9_refute_ec2_remote.sh \
  --run-id '$RUN_ID' --work '/root/run9-frozen-refute-$RUN_ID' \
...
instance=$("${aws_cmd[@]}" ec2 run-instances --region "$REGION" \
    ...
    --tag-specifications \
      "ResourceType=instance,Tags=[{Key=Project,Value=radio-sa193},{Key=Purpose,Value=run9-frozen-refute},{Key=RunId,Value=$RUN_ID},{Key=Name,Value=run9-frozen-refute-$RUN_ID}]" \
      "ResourceType=volume,Tags=[{Key=Project,Value=radio-sa193},{Key=Purpose,Value=run9-frozen-refute},{Key=RunId,Value=$RUN_ID}]" \
    --query 'Instances[0].InstanceId' --output text)

printf 'launched instance=%s type=%s run_id=%s commit=%s\n' "$instance" "$TYPE" "$RUN_ID" "$commit"
printf 's3=s3://%s/%s/\nstatus=tools/run9_refute_status.sh %s\n' "$BUCKET" "$prefix" "$RUN_ID"
